  ...
  chrome.fileSystem.chooseEntry({type: 'openFile', accepts: accepts}, function(readOnlyEntry) {
    ...
    chosenFileEntry.file(function(file) {
      readAsText(readOnlyEntry, function(result) {
		try {
		  var c = $.parseJSON(result);
		  loadnewstage(result);
		}
		catch (err) {
		  // Do something about the exception here
		  // Show Error on screen
		  var errorstr="There was an Error opening the file. The most common problem is that the file was not generated by the block diagram tool.<br>";
		  $( "#dialog-form2" ).html(errorstr);
...
{
  var config = {type: 'saveFile', suggestedName: 'blockdiagram.bdt'};
...
{
  var config = {type: 'savejpg', suggestedName: 'blockdiagram.png'};
  chrome.fileSystem.chooseEntry(config, function(entry) {
...
document.addEventListener('DOMContentLoaded', function() {
    var link = document.getElementById('savepdf');
    // onClick's logic below:
    link.addEventListener('click', function() {
        saveFilePDF();
    });
});
function saveFilePDF(pdfdata)
{
  var config = {type: 'savepdf', suggestedName: 'blockdiagram.pdf'};
  chrome.fileSystem.chooseEntry(config, function(entry) {
	pdfdata = getpdf();
...
document.addEventListener('DOMContentLoaded', function() {
    var link = document.getElementById('savepaper');
    // onClick's logic below:
    link.addEventListener('click', function() {
        window.print();
    });
